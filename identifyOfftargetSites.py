...
def analyze(sam_filename, reference_genome, outfile, annotations):
    ...
    for line in file:
        ...
        if len(fields) >= 10:
            ...
            if int(mapq) >= 50 and int(sam_flag) & 64 and not int(sam_flag) & 2048:
                # Second read in pair
...
def assignPrimerstoReads(read_sequence, sam_flag):
    ...
    if int(sam_flag) & 16:
        ...
        readstart = read_sequence[3:23]
    if readstart == "CCGTTATTAACATATGACAA":
#        logger.info("primer1")
        return "primer1"
    elif readstart == "TTGTCATATGTTAATAACGG":
#        logger.info("primer2")
        return "primer2"
    else:
#        logger.info("nomatch: %s %s", read_sequence, readstart)
        return "nomatch"
